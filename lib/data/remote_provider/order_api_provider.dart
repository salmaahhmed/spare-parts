    ...
    if (supplierSpareParts != null) {
      for (ParseObject value in supplierSpareParts) {
        ...
        List<ParseObject> list =
            getApiResponse(await queryOrderSupplierSpareParts.query())
                    ?.results ??
                [];
        list?.forEach((item) async {
          ParseObject supplierSparePart = item.get("supplier_spare_part_id");
          ParseObject sparePart = await fetchParseObjectIfNeeded<ParseObject>(
              supplierSparePart.get("spare_part_id"));
          item.set("supplier_spare_part_id",
              supplierSparePart..set("spare_part_id", sparePart));
          ordersList.add(item);
        });
      }
    }
